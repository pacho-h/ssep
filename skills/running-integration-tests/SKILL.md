...
description: Authors and executes tests across all levels of the test pyramid — unit, frontend-backend integration, and end-to-end browser scenarios — using Playwright MCP for live browser control and standard test runners (jest, vitest, mocha) for code-level verification. Use when verifying multi-layer behavior, when user requests "e2e 테스트", "integration 테스트", "통합 테스트", "playwright로 검증", "브라우저에서 직접 확인", "백엔드-프론트 연동 테스트", "API 통합 테스트", or when a feature crosses service boundaries (API + UI + DB) and isolated unit tests cannot prove the system actually works together. Helps decide which test level fits which scenario, sets up the appropriate test infrastructure, writes the tests, runs them, and reports failures with diagnostic context. Distinct from superpowers:test-driven-development which focuses on unit-level TDD discipline; this skill specifically handles the integration and e2e tiers and the decision of when each tier applies.
